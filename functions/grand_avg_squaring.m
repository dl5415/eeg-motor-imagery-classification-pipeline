...
ga(:,:,1) = [squeeze(mean(signal(idx_flex,:,channelSelect).^2)) squeeze(std(signal(idx_flex,:,channelSelect).^2)/sqrt(length(idx_flex)))];
ga(:,:,2) = [squeeze(mean(signal(idx_ext,:,channelSelect).^2)) squeeze(std(signal(idx_flex,:,channelSelect).^2)/sqrt(length(idx_ext)))];
ga(:,:,3) = [squeeze(mean(signal(idx_rest,:,channelSelect).^2)) squeeze(std(signal(idx_rest,:,channelSelect).^2)/sqrt(length(idx_rest)))];
end
